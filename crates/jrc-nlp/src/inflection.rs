...
/// - A prefix absorbs the following noun (低＋声) and that noun's
///   suffixes.
...
fn phrase_end(tokens: &[Token], start: usize) -> usize {
    ...
    match head.pos {
        ...
        PartOfSpeech::AdjectivalNoun | PartOfSpeech::Noun | PartOfSpeech::ProperNoun => {
            end = absorb_noun_suffixes(tokens, end);
        }
        // 低＋声, 全＋世界: a prefix binds to the following noun.
        PartOfSpeech::Prefix
            if end < tokens.len()
                && matches!(
                    tokens[end].pos,
...
/// Absorb directly attached noun suffixes: 日本語＋版, 田中＋さん.
fn absorb_noun_suffixes(tokens: &[Token], mut end: usize) -> usize {
...
mod tests {
    ...
    #[test]
    fn prefix_binds_to_following_noun() {
        // 低声 (ていせい) is analyzed as prefix 低 + noun 声; the reader
        // should treat it as one unit.
        let (surface, inflection) = group_surface("老爺は低声で答えた。", "低");
        assert_eq!(surface, "低声");
        assert!(inflection.summary.is_none());

        let (surface, _) = group_surface("全世界が驚いた。", "全");
        assert_eq!(surface, "全世界");
    }
