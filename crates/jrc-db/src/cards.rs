...
use jrc_srs::{Card, CardState, Rating};
use rusqlite::params;
...
mod tests {
    ...
    use jrc_core::{KnowledgeStatus, PartOfSpeech, WordKey};
    ...
    fn card_roundtrip_and_due_query() {
        ...
        let card_due = scheduler.review(&Card::new(now), jrc_srs::Rating::Again, now);
        let card_future = scheduler.review(&Card::new(now), jrc_srs::Rating::Easy, now);
        ...
        let updated = scheduler.review(&card_due, jrc_srs::Rating::Good, soon);
