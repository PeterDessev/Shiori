...
use chrono::Utc;
use jrc_core::{KnowledgeStatus, Token, WordId, WordKey};
use jrc_db::WordRow;
use jrc_srs::Card;
    ...
    pub fn analyze_chat_text(
        ...
        for paragraph in &analyzed.paragraphs {
            for sentence in &paragraph.sentences {
                ...
                let groups = jrc_nlp::phrase_groups(&sentence.tokens);
                let tokens = sentence
                    .tokens
                    .iter()
                    .map(|t| {
...
mod tests {
    ...
    fn ensure_word_creates_once() {
        let app = app();
        let key = WordKey::new("勉強", "べんきょう", jrc_core::PartOfSpeech::Noun);
