...
use jrc_db::anki::{AnkiNote, AnkiSchedule, ImportedNote};
use jrc_srs::{Card, CardState};
...
impl App {
    ...
    pub fn export_apkg(&self, path: &Path) -> Result<usize> {
        ...
        let count = notes.len();
        jrc_db::anki::write_apkg(path, "Japanese Reading Companion", &notes)?;
        Ok(count)
    }
    ...
    pub fn import_apkg(&self, path: &Path) -> Result<(usize, usize)> {
        let notes = jrc_db::anki::read_apkg(path)?;
        let now = Utc::now();
        let mut imported = 0;
        let mut skipped = 0;
        for note in &notes {
    ...
    fn import_note(&self, note: &ImportedNote, now: chrono::DateTime<Utc>) -> Result<bool> {
        ...
        let Some(expression) = note
            ...
            .find(|f| jrc_nlp::kana::is_japanese(f.trim()))
        else {
            return Ok(false);
        };
...
mod tests {
    ...
    fn apkg_export_import_roundtrip() {
        ...
        let word = app
            .ensure_word(&jrc_core::WordKey::new(
                "勉強",
                "べんきょう",
        ...
        let word = other
            .db()
            .find_word(&jrc_core::WordKey::new(
                "勉強",
                "べんきょう",
                jrc_core::PartOfSpeech::Noun,
            ))
