...
Reading time comes from the reading-session clock, which runs while a book is open in the reader and is cleaned by the away rules: pausing stops the clock, an auto-away page is credited at most 1× its expected reading time, and pages flipped through too fast are excluded entirely. See [Reading](@/docs/reading.md) for the exact rules. Until you have recorded any time, this section shows a note that the clock runs while a book is open.
