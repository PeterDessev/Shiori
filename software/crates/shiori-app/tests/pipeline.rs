...
use shiori_db::Db;
use shiori_srs::{CardState, Rating};
...
fn honorific_register_is_surfaced() {
    ...
    let profile = shiori_dict::register::UsageProfile::from_misc_codes(entry.misc_codes());
    assert_eq!(
        profile.registers,
        vec![shiori_dict::register::Register::Honorific]
    );
}
